...
def _async_setup_entities(devices, async_add_entities):
    ...
    for dev in devices:
        if DEV_TYPE_TO_HA.get(dev.device_type) == "humidifier":
            ...
            dev_list.append(VeSyncHumidifierFanHA(dev))
        else:
            _LOGGER.warning(
...
class VeSyncHumidifier(VeSyncDevice):
    @property
    def is_on(self) -> bool:
    ...
    def device_info(self):
        return {
            "connections": {
                (dr.CONNECTION_NETWORK_MAC, self.device.mac_id),
            },
            "identifiers": {
...
class VeSyncHumidifierFanHA(VeSyncHumidifier, FanEntity):
    @property
...
class VeSyncHumidifierHA(VeSyncHumidifier, HumidifierEntity):
    ...
    def unique_info(self):
        """Return the ID of this humidifier."""
        return super().unique_info() + "humidifier"
    ...
    # TODO Nur humidifier Attributes
    # TODO attirubtes auch zu anderen hinzufügen
    @property
